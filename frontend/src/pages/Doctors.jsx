  ...
  return (
    <div>
      ...
      <div className='flex flex-col sm:flex-row items-start gap-5 mt-5'>
        ...
        <div className={`  flex-col gap-4 text-sm text-gray-800 ${showFilter? 'flex' : 'hidden sm:flex'}`}>
          <p onClick={()=>
            speciality==='General physician'? navigate('/doctors'): navigate('/doctors/General physician')}
            className={`w-[94vw] sm:w-auto pl-3 py-1.5 pr-16 border border-gray-300 rounded transition-all cursor-pointer
             ${speciality==='General physician' ?'bg-green-100 text-black' : ''}`}>
              General physician</p>
        ...
        <div className='w-full grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4 gap-y-6'>
          ...
          <div onClick={()=>navigate(`/appointment/${item._id}`)} className='border border-green-200 rounded-xl overflow-hidden cursor-pointer hover:translate-y-[-10px] transition-all duration-500' key={index}>
            <img className='bg-green-50 w-full aspect-[3/4] object-cover object-top' src={item.image} alt="" />
            <div className='p-3'>
