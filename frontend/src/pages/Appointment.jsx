...
const slotDate= date? date.getDate() +"_" + (date.getMonth()+1) + "_" +date.getFullYear() : "";
...
<p
 key={item.time}
  onClick={
    () => {
       console.log("Clicked", item.time)
      if(docInfo?.slots_booked?.[slotDate]?.includes(item.time)){
        toast.error("Slot is already booked")
      }else{
        setSlotTime(item.time.toLowerCase())
      }
    }
  } className={`text-sm font-light flex-shrink-0 px-5 py-2 rounded-full cursor-pointer z-10 
  ${item.time?.trim().toLowerCase()===slotTime?.trim().toLowerCase() ? 'bg-primary text-white' : ' text-gray-500 border border-gray-300'}`} >
